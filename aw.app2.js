...
//var supervisorname = ""; kanske inte behövs
//var subjectname = ""; kanske inte behövs
...
/***function writeNames(){
  ...
  arrayIndex = awapp.writeByte(lengthName);

  let charToWrite = 0;
  for(let i=0; i<lengthName;i ++) {
    charToWrite = supervisorname[i].charCodeAt(0);
    arrayIndex = awapp.writeByte(charToWrite);
  if(awapp.hitLimit() == true){
      writeToFlash();
    }
  }
...
function writeConfiguration(){
  ...
  require("Storage").erase("collectedData.bin");
  ...
  awapp.writeOptions1(recordAltitude,jsMath, outputXML);
  awapp.writeOptions2(filenameDate);
  var time = Math.round(Date.now());
  lastTimestamp = time;
  var date = Date(time);
  awapp.writeDate(date.getFullYear(), date.getMonth(), date.getDate());
  var writeOut = awapp.writeTime(date.getHours(), date.getMinutes());
